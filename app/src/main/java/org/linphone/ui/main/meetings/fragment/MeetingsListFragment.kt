    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.setNewMeetingClicked {
            if (findNavController().currentDestination?.id == R.id.meetingsListFragment) {
                ...
                val action =
                    MeetingsListFragmentDirections
                        .actionMeetingsListFragmentToScheduleMeetingFragment()
        ...
        /*
         * Added this manually like ConversationsListFragment.
         * This is needed because we are not calling initViews().
         */
...
//            }
        ...
        adapter.meetingClickedEvent.observe(viewLifecycleOwner) {
            it.consume { model ->
                if (model.isCancelled) {
                    Log.w(
                        "$TAG Meeting with ID [${model.id}] is cancelled, can't show the details"
                    )
                } else {
                ...
                val modalBottomSheet = MeetingsMenuDialogFragment(
                    ...
                    },
                    {
                        ...
                        if (showCancelActionInsteadOfDelete) {
                            Log.i(
                                "$TAG Meeting start hasn't started yet and we are the organizer, asking user if it should be cancelled"
                            )
        ...
        sharedViewModel.goToMeetingWaitingRoomEvent.observe(viewLifecycleOwner) {
            it.consume { uri ->
                if (findNavController().currentDestination?.id == R.id.meetingsListFragment) {
                    ...
                    val action =
                        MeetingsListFragmentDirections
                            .actionMeetingsListFragmentToMeetingWaitingRoomFragment(uri)
         ...
         * )
         *
         * Removing initViews() removes default top bar, side menu,
         * and bottom navigation from Meetings list.
         *
         * This makes MeetingsListFragment work similar to ConversationsListFragment.
         */
    ...
    /*
     * Added similar to ConversationsListFragment.
     * Meeting details will open inside meetingsNavContainer child NavHost,
     * and SlidingPaneLayout will open manually.
     */
    private fun openMeeting(meetingId: String) {
        try {
            val action =
                MeetingFragmentDirections.actionGlobalMeetingFragment(meetingId)

            val meetingsNavController = binding.meetingsNavContainer.findNavController()

            Log.i(
                "$TAG Opening MeetingFragment using meetingsNavContainer NavController, meetingId=[$meetingId]"
            )

            meetingsNavController.navigate(action)

            if (!binding.slidingPaneLayout.isOpen) {
                Log.i("$TAG Opening sliding pane to show meeting details")
                binding.slidingPaneLayout.openPane()
            }
        } catch (e: IllegalArgumentException) {
            Log.e("$TAG Failed to open MeetingFragment: $e")
        } catch (e: IllegalStateException) {
            Log.e("$TAG Failed to open sliding pane / meeting nav controller: $e")
        }
    }
    ...
    private fun scrollToToday() {
        ...
        if (index >= 0) {
            // Workaround to have header decoration visible at top
            binding.meetingsList.smoothScrollToPosition(index)

            (binding.meetingsList.layoutManager as LinearLayoutManager)
    ...
    private fun showCancelMeetingDialog(meetingModel: MeetingModel) {
        ...
        model.confirmEvent.observe(viewLifecycleOwner) {
            it.consume {
                Log.i(
                    "$TAG Cancelling meeting [${meetingModel.id}] and sending notification to participants"
                )

                meetingViewModelBeingCancelled = meetingModel
                listViewModel.cancelMeetingViewModel.cancelMeeting(
                    meetingModel.conferenceInfo,
                    true
                )
        ...
        model.alternativeChoiceEvent.observe(viewLifecycleOwner) {
            it.consume {
                Log.i(
                    "$TAG Cancelling meeting [${meetingModel.id}] without notifying participants"
                )
